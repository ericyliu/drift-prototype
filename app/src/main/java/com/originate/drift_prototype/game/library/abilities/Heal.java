...
public class Heal extends Ability {
    ...
    public String invoke(Character source, Character target) {
        ...
        amount += source.stats.get("intelligence").current * .50;
        target.receiveHealth(amount);

        if (source.id == target.id) return String.format("%s has healed themselves for %d",
                                                         source.name, MoreMath.doubleToInt(amount));
        else return String.format("%s has healed %s for %d",
